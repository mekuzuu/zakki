...
相変わらずまったり読んでおり、第6章を読んだ。 

リポジトリについてのお話。

- リポジトリはデータを永続化（保存）し再構築（復元）したりするためのもの。
- リポジトリを利用することで、ソフトウェアに柔軟性をもたらす。
- なぜなら、特定のインフラストラクチャ技術に依存することで複雑化してしまうため。
- 例えば、ドメインのコードの大半が永続化に関する操作で汚染されると、ドメインのロジックや目的が曖昧になってしまう。
- リポジトリを利用することで、データの永続化や再構築にまつわる処理を抽象化することができる。
- つまり、ドメインをシンプルに保てる。
- また、Interfaceで抽象化することでインフラストラクチャ技術の変更容易性をもたらす。