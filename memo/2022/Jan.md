...
とりあえず公式ドキュメントをざっと読んだら、何か作るのをゴールに学習するほうが定着しそうだなと思っている。

## Rust

パッケージ、クレート、モジュールについてのメモ。

* パッケージはある機能群を提供する1つ以上のクレートで、`Cargo.toml`で管理される単位
* クレートはモジュールの集まりで、コンパイル可能な単位
* モジュールは`mod`で`use`で扱われる単位
